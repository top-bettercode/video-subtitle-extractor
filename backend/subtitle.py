#!/home/peter/.local/share/virtualenvs/video-subtitle-extractor-av5uNC_5/bin/python
# -*- coding: utf-8 -*-
"""
@Author  : Fang Yao
...
class SubtitleExtractor:
    ...
    def __init__(self, vd_path, sub_area=None, default_subtitle_area=config.DEFAULT_SUBTITLE_AREA):
        importlib.reload(config)
        # 线程锁
        ...
        # 临时存储文件夹
        self.temp_output_dir = os.path.join(tempfile.gettempdir(), 'video-subtitle-extractor', 'output', str(self.vd_name))
        # 视频帧总数
        ...
        self.default_subtitle_area = default_subtitle_area
        # 提取的视频帧储存目录
    ...
    def run(self):
        ...
        if self.sub_area is None and (self.default_subtitle_area == SubtitleArea.UNKNOWN or self.default_subtitle_area == SubtitleArea.CUSTOM):
    ...
    def extract_frame_by_vsf(self):
       ...
       """
        ...
        def vsf_output(out, ):
            ...
            for line in iter(out.readline, b''):
                ...
                if line.startswith('Frame: '):
                    ...
                    print('VideoSubFinderCli:', line.strip())
            out.close()
        ...
        if platform.system() == 'Windows':
            # 定义执行命令
            cmd = f"{path_vsf} --use_cuda -c -r -i \"{get_absolute_path(self.video_path)}\" -o \"{self.temp_output_dir}\" -ces \"{self.vsf_subtitle}\" "
            ...
            # 定义执行命令
            cmd = f"{path_vsf} -c -r -i \"{get_absolute_path(self.video_path)}\" -o \"{self.temp_output_dir}\" -ces \"{self.vsf_subtitle}\" "
            if config.USE_GPU:
                cmd += "--use_cuda "
            cmd += f"-te {top_end} -be {bottom_end} -le {left_end} -re {right_end} -nthr {cpu_count} -dsi -ovffmpeg"
            self.vsf_running = True
            import subprocess
    ...
    def filter_scene_text(self):
        ...
        if not ret:
            ...
            return
        cropped = 0
        if self.default_subtitle_area == SubtitleArea.LOWER_PART:
            cropped = int(sample_frame.shape[0] * 3 // 4)
        ...
        # 画出字幕框的区域
        cv2.rectangle(sample_frame, pt1=(0, ymin + cropped), pt2=(sample_frame.shape[1], ymax + cropped), color=(0, 0, 255), thickness=3)
...
def get_absolute_path(input_path):
    ...
    absolute_path = os.path.abspath(os.path.join(current_directory, input_path))
...
if __name__ == '__main__':
    ...
    if len(sys.argv) > 2:
        ...
        subtitle_area = (y_min, y_max, x_min, x_max)
        default_subtitle_area = SubtitleArea.CUSTOM
    else:
        ...
        video_cap.release()
        # 下部分字幕区域
        subtitle_area = (int(frame_height * 3 // 4), frame_height, 0, frame_width)
        # 上部分字幕区域
        # subtitle_area = (0, int(frame_height * 1 // 4), 0, frame_width)
        default_subtitle_area = SubtitleArea.UNKNOWN
        # default_subtitle_area = SubtitleArea.LOWER_PART
        # subtitle_area = None


    # 新建字幕提取对象
    se = SubtitleExtractor(vd_path=video_path, sub_area=subtitle_area, default_subtitle_area=default_subtitle_area)
    # 开始提取字幕
    se.run()
